            ...
            <div className="main flex flex-col items-center relative">
                ...
                <div className="search">
                    ...
                    <div className="relative">
                        ...
                        <input type="search" id="default-search" className="block w-[50%] lg:w-[46%] p-4 pl-10 text-sm bg-[#021d21] border-gray-600 placeholder-gray-400 text-whitee focus:outline-none rounded-lg" placeholder="Search For A Country..." onChange={(e) => e.target.value.length === 0 ? setCountries(Info) : setCountries(countries.filter(c => c.country.toLowerCase().includes(e.target.value.toLowerCase())))} required />
                        <button onClick={() => setOpen(true)} type="button" className="flex justify-center items-center gap-2 absolute right-0 top-0 text-white bg-[#ffffff0d] focus:ring-4 focus:ring-[#021d21] focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 w-[255px] pl-3 text-center mr-2 mb-2" style={{
                            paddingLeft: '2.5rem',
                            padding: '0.8rem',
                            fontSize: '21px',
